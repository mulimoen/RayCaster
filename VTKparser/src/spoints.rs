...
pub fn parse<R: io::Read>(reader: &mut io::BufReader<R>,
                          binary: bool)
    ...
    if !dims.starts_with("DIMENSIONS ") {
        return Err(VTKparseError::FileFormat("Does not contain dimensions".to_string()));
    }
    ...
    if !origin.starts_with("ORIGIN ") {
        return Err(VTKparseError::FileFormat("Does not contain origin".to_string()));
    }
    ...
    if !spacing.starts_with("SPACING ") && !spacing.starts_with("ASPECT_RATIO ") {
        return Err(VTKparseError::FileFormat("Does not contain spacing".to_string()));
    }
    ...
    if !line.starts_with("POINT_DATA ") {
        return Err(VTKparseError::FileFormat("Does not contain length".to_string()));
    }
    ...
    if !line.starts_with("SCALARS ") {
        return Err(VTKparseError::FileFormat("Does not contain scalar type".to_string()));
    }
    ...
    if datatype != "unsigned_char" {
        return Err(VTKparseError::NotImplemented(format!("Datatype {}", datatype).to_string()));
    }

    if !binary {
        return Err(VTKparseError::NotImplemented("ASCII extraction".to_string()));
    }
    ...
    if colours.len() != len {
        return Err(VTKparseError::UnknownFormat(format!("Number of elements in binary is not correct. Expected {} but got {}",
                                                        len,
                                                        colours.len())));
    }
...
fn get_dimensions(dims: &str) -> Result<(u32, u32, u32), VTKparseError> {
    ...
    match (d0, d1, d2) {
        ...
        _ => Err(VTKparseError::FileFormat("Can not extract dimensions".to_string())),
    }
}
...
fn get_origo(origin: &str) -> Result<(f32, f32, f32), VTKparseError> {
    ...
    match (d0, d1, d2) {
        (Some(x), Some(y), Some(z)) => Ok((x.parse()?, y.parse()?, z.parse()?)),
        _ => Err(VTKparseError::FileFormat("Can not extract origin".to_string())),
    }
}
